...
var stickywidth = 350; // width of sticky note (can't be less than 200)
var stickyheight = 350; // height of sticky note (can't be less than 200)
...
html5sticky.addNote = function () {
   ...
   $('.note_common p').css({
      height: (stickyheight - 80) + 'px',
      width: (stickywidth - 20) + 'px'
   });
...
html5sticky.addUtilityButtons = function (el) {
   ...
   $('.icons-footer').append('<img class="left shrink ficon" src="sticky/assets/img/shrink.png" alt="" title="shrink/expand">');
   $('.icons-footer').append('<img class="left alarm ficon" src="sticky/assets/img/alarm.png" alt="" title="set alarm for this sticky note">');
   $('.icons-footer').append('<img class="left lock ficon" src="sticky/assets/img/lock.png" alt="" title="lock this sticky note">');
   $('.icons-footer').append('<img class="left settings ficon" src="sticky/assets/img/settings.png" alt="" title="settings for note">');
   $('.icons-footer').append('<img class="left email ficon" src="sticky/assets/img/email.png" alt="" title="email this sticky note">');
   $('.icons-footer').append('<img class="left tweet ficon" src="sticky/assets/img/tweet.png" alt="" title="tweet this sticky note">');
   $('.icons-footer').append('<img class="left share ficon" src="sticky/assets/img/share.png" alt="" title="share this sticky note">');
...
html5sticky.editNote = function ($clone, el) {
   ...
   $clone.find('p').replaceWith('<div class="row"><textarea class="pedit" /></div');
   ...
   $clone.find('h2').replaceWith('<div class="row"><input type="text" class="hedit" /></div>');

   $('.hedit').addClass('inset').val(html5sticky.stripTags(htext, allowed_tags)).width(250);


   // put in Close button
   $('<a href="#" class="close_stickynote"><img src="sticky/assets/img/deletebig.png" alt="" title="close note"></a>')
      .css({
         position: 'absolute',
         top: 7,
         right: 5
   ...
   // put in Save button
   $('<a href="#" class="save_stickynote"><img src="sticky/assets/img/savebig.png" alt="" title="save note"></a>')
      .css({
         position: 'absolute',
         top: 7,
         right: 60
...
html5sticky.addPin = function (el) {
   var close = $('<div><a href="#" class="delete_stickynote"><img src="sticky/assets/img/delete.png" alt="" title="delete note"></a></div>');
   var tag = $('<div align="center"><img src="sticky/assets/img/pin.png" alt=""></div>');
...
html5sticky.enlargeNote = function (el) {
   ...
   if (Modernizr.localstorage) {
      ...
      var timeImg = '<img class="left" align="absmiddle" src="sticky/assets/img/time.png">';
...
$(function () {
   ...
   // delete note
   $(document).on("click", ".delete_stickynote", function () {
      html5sticky.deleteNote($(this));
      return false;
   });

   // close enlarged note
   $(document).on("click", ".close_stickynote", function () {
      html5sticky.closeNote($(this));
      return false;
   });

   // save the note
   $(document).on("click", ".save_stickynote", function () {
      html5sticky.saveNote($(this));
      return false;
   });

   // enlarge the note
   $(document).on("click", ".note_common", function () {
